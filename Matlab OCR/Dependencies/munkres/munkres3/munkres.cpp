...
Munkres::step4(void) {
	...
	*/
	...
	for ( std::list<std::pair<int,int> >::iterator iter = seq.begin() ;
		  iter != seq.end() ;
		  iter++ ) {
		/* 2. Unstar each starred zero of the sequence. */
		if ( mask_matrix(iter->first,iter->second) == Z_STAR )
			mask_matrix(iter->first,iter->second) = Z_NORMAL;

		/* 3. Star each primed zero of the sequence, 
			thus increasing the number of starred zeros by one. */
		if ( mask_matrix(iter->first,iter->second) == Z_PRIME )
			mask_matrix(iter->first,iter->second) = Z_STAR;
	}
