 ...
 *
 *
 *   Change matrix.cpp to my_matrix.cpp to resolve conflicts with Matlab's own matrix.h
 */
...
Matrix<T>::resize(int rows, int columns) {
	if ( m_matrix == NULL ) {
		...
		int minrows = rows<m_rows?rows:m_rows; /*  std::min<int>(rows, m_rows); */
		int mincols = columns<m_columns?columns:m_columns; /* std::min<int>(columns, m_columns); */
